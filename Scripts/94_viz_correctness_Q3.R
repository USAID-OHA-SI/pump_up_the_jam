...
# IMPORT ------------------------------------------------------------------
  ...
  file_name <- paste0("HFR_Correctness_", quarter, "_[[:digit:]]+\\.csv")
    df_completeness_viz <- list.files(out_folder, file_name, full.names = TRUE) %>% 
      vroom(., col_types = c(ou_annotation = "d"))
...
# PLOT --------------------------------------------------------------------
  ...
  #select OUs - removing South Africa for Q3
    top_ous <- df_datim_wgts %>% 
      filter(operatingunit != "South Africa") %>% 
      group_by(operatingunit, indicator) %>% summarise(targets = sum(mer_targets, na.rm = T)) %>% 
      spread(indicator, targets) %>% arrange(desc(TX_CURR)) %>% ungroup() %>% 
      slice(1:10) %>% pull(operatingunit)
